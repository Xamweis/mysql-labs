...
#####################################################
-- ALIASING is possible WITH and WITHOUT keword AS
#####################################################

-- 1) nums of films in categories
SELECT category.name AS category, COUNT(*) AS count
FROM category LEFT JOIN film_category ON film_category.category_id = category.category_id
...
WHERE CONVERT(rental_date, DATE) > CONVERT('2005-07-31', DATE) 
AND CONVERT(rental_date, DATE) < CONVERT('2005-09-01', DATE);
-- same as
...
-- same as
SELECT DISTINCT f.title, COUNT(*) OVER (PARTITION BY f.title) AS count
